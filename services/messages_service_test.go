...
func TestMessagesService_GetMessage_Success(t *testing.T) {
	...
	assert.EqualValues(t, "title", msg.Title)
...
func TestMessagesService_CreateMessage_Success(t *testing.T) {
	domain.MessageRepo = &getDBMock{}
	createMessageDomain = func(message *domain.Message) (*domain.Message, error_utils.MessageErr) {
		return &domain.Message{
			Id:        1,
			Title:     "title",
			Body:      "body",
...
func TestMessagesService_UpdateMessage_Success(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.EqualValues(t, 1, msg.Id)
	assert.EqualValues(t, "title updated", msg.Title)
	assert.EqualValues(t, "body updated", msg.Body)
}
